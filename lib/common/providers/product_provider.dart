...
class ProductProvider extends DataSyncProvider {
  ...
  Future<void> getLatestProductList(int offset, bool reload,
      {bool isUpdate = true}) async {
    final ProductSortProvider productSortProvider =
        Provider.of<ProductSortProvider>(Get.context!, listen: false);
    ...
    if (offset == 1) {
      fetchAndSyncData(
        fetchFromLocal: () => productRepo!
            .getLatestProductList<CacheResponseData>(
                offset: offset,
        ...
        if (response?.response?.data != null &&
          ...
          if (offset == 1) {
            ...
            _latestProductModel?.products?.addAll(
                ProductModel.fromJson(response?.response?.data).products ?? []);
          }
          notifyListeners();
        } else {
  ...
  Future<void> getPopularLocalProductList(int offset, bool reload,
      {bool isUpdate = true,
    ...
    if (offset == 1) {
      fetchAndSyncData(
        fetchFromLocal: () => productRepo!
            .getPopularProductList<CacheResponseData>(
                offset: offset, source: DataSourceEnum.local),
        fetchFromClient: () => productRepo!.getPopularProductList(
      ...
      ApiResponseModel? response = await productRepo?.getPopularProductList(
          offset: offset, source: DataSourceEnum.client);

      if (response?.response?.data != null &&
          response?.response?.statusCode == 200) {
        if (offset == 1) {
          _popularLocalProductModel =
              ProductModel.fromJson(response?.response?.data);
        } else {
          _popularLocalProductModel?.totalSize =
              ProductModel.fromJson(response?.response?.data).totalSize;
          _popularLocalProductModel?.offset =
              ProductModel.fromJson(response?.response?.data).offset;
          _popularLocalProductModel?.products?.addAll(
              ProductModel.fromJson(response?.response?.data).products ?? []);
        }
  ...
  Future<void> getRecommendedProductList(int offset, bool reload,
      {bool isUpdate = true}) async {
    if (reload) {
    ...
    if (offset == 1) {
      fetchAndSyncData(
        fetchFromLocal: () => productRepo!
            .getRecommendedProductApi<CacheResponseData>(
                offset: offset, source: DataSourceEnum.local),
      ...
      if ((_recommendedProductModel == null) || (offset != 1)) {
        ApiResponseModel? response =
            await productRepo?.getRecommendedProductApi(
                offset: offset, source: DataSourceEnum.client);
        if (response?.response?.data != null &&
            response?.response?.statusCode == 200) {
          debugPrint(
          ...
          if (offset == 1) {
            ...
            _recommendedProductModel?.totalSize =
                ProductModel.fromJson(response?.response?.data).totalSize;
            _recommendedProductModel?.offset =
                ProductModel.fromJson(response?.response?.data).offset;
            _recommendedProductModel?.products?.addAll(
                ProductModel.fromJson(response?.response?.data).products ?? []);
          }
  ...
  Future<void> getFlavorfulMenuProductMenuList(int offset, bool reload,
      {bool isUpdate = true}) async {
    if (reload) {
    ...
    if (offset == 1) {
      fetchAndSyncData(
        fetchFromLocal: () => productRepo!
            .getFlavorFulMenuProductApi<CacheResponseData>(
                offset: offset, source: DataSourceEnum.local),
        fetchFromClient: () => productRepo!.getFlavorFulMenuProductApi(
            offset: offset, source: DataSourceEnum.client),
        onResponse: (responseData, _) {
      ...
      if (_flavorfulMenuProductMenu == null || offset != 1) {
        ApiResponseModel? response =
            await productRepo?.getFlavorFulMenuProductApi(
                offset: offset, source: DataSourceEnum.client);

        if (response?.response?.data != null &&
            response?.response?.statusCode == 200) {
          if (offset == 1) {
            _flavorfulMenuProductMenu =
                ProductModel.fromJson(response?.response?.data);
          } else {
            _flavorfulMenuProductMenu?.totalSize =
                ProductModel.fromJson(response?.response?.data).totalSize;
            _flavorfulMenuProductMenu?.offset =
                ProductModel.fromJson(response?.response?.data).offset;
            _flavorfulMenuProductMenu?.products?.addAll(
                ProductModel.fromJson(response?.response?.data).products ?? []);
          }
  ...
  void checkIsRequiredSelected(
      {required int index,
      required bool isMultiSelect,
      int? min = 1,
      int? max = 1,
      required List<bool?> variations}) {
    if (isMultiSelect) {
      int count = 0;
  ...
  Future<ReorderProductModel?> getReorderProductList(int? orderId,
      {isUpdate = true}) async {
    _isLoading = true;
    ...
    ApiResponseModel? response =
        await productRepo?.getReorderProductApi(orderId);

    if (response?.response?.data != null &&
        response?.response?.statusCode == 200) {
      _reorderProductModel =
          ReorderProductModel.fromJson(response?.response?.data);
      notifyListeners();
    } else {
      ApiCheckerHelper.checkApi(response!);
    }
  ...
  Future<void> getReviewData({int? id, int? offset}) async {
    if (_reviewModel == null || offset != 1) {
      ApiResponseModel response =
          await productRepo!.getReview(id: id, offset: offset);
      if (response.response!.statusCode == 200) {
        if (offset == 1) {
          _reviewModel = ReviewModel.fromJson(response.response?.data);
        } else {
          _reviewModel?.reviews?.totalSize =
              ReviewModel.fromJson(response.response?.data).reviews?.totalSize;
          _reviewModel?.reviews?.offset =
              ReviewModel.fromJson(response.response?.data).reviews?.offset;
          _reviewModel?.reviews?.reviewList?.addAll(
              ReviewModel.fromJson(response.response?.data)
                      .reviews
                      ?.reviewList ??
                  []);
        }
