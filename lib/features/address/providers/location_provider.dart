...
class LocationProvider with ChangeNotifier {
  ...
  Future<bool> checkPermission(Function callback,
      {bool canBeIgnoreDialog = false}) async {
  ...
  Future<AddressModel?> getCurrentLocation(BuildContext context, bool isUpdate,
  ...
  // update Position
  Future<void> updatePosition(CameraPosition? position, bool fromAddress,
      String? address, BuildContext context, bool forceNotify,
  ...
  // delete user address
  Future<void> deleteUserAddressByID(
      int? id, int index, Function callback) async {
    _isLoading = true;
    notifyListeners();

    ApiResponseModel apiResponse = await locationRepo!.removeAddressByID(
        id: id,
  ...
  Future<List<AddressModel>?> initAddressList() async {
    ...
    if (apiResponse.response != null &&
      ...
      if (_currentAddress?.id != null &&
          !addressIds.contains(_currentAddress?.id)) {
  ...
  Future<ResponseModel> addAddress(AddressModel addressModel) async {
    ...
    if (apiResponse.response != null &&
      ...
      String? message =
          getTranslated("address_added_successfully", Get.context!);
  ...
  Future<String> getAddressFromGeocode(LatLng latLng) async {
    ...
    if (response.response?.statusCode == 200 &&
      ...
      final String status =
          response.response?.data['status']?.toString() ?? 'unknown';
      debugPrint('--------address_api_key_status----$status');
      ...
      try {
        ...
        debugPrint('--------address_fallback_failed----$e');
      }
    }
    return address;
  }
  ...
  void _onStoreCurrentLocation(AddressModel? address) {
    try {
      sharedPreferences.setString(
          AppConstants.currentAddress, jsonEncode(address?.toJson()));
    } catch (e) {
      rethrow;
    }
  }
  ...
  AddressModel? getCachedCurrentLocation() {
    final currentAddressAsString =
        sharedPreferences.getString(AppConstants.currentAddress);
